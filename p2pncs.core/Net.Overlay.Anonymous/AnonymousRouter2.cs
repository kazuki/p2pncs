...
using AnonymousEndPoint = p2pncs.Net.Overlay.Anonymous.AnonymousRouter.AnonymousEndPoint;
using DHTEntry = p2pncs.Net.Overlay.Anonymous.AnonymousRouter.DHTEntry;

namespace p2pncs.Net.Overlay.Anonymous
{
	public class AnonymousRouter2 : IAnonymousRouter
	{
		...
		public IAsyncResult BeginEstablishRoute (Key recipientId, Key destinationId, DatagramReceiveEventHandler receivedHandler, AsyncCallback callback, object state)
		{
			...
			using (IDisposable cookie = _connectionMapLock.EnterWriteLock ()) {
				...
				info = new ConnectionInfo (this, subscribeInfo, destinationId, id, receivedHandler, callback, state);
				...
				if (msg != null) {
					...
					using (IDisposable cookie = _connectionMapLock.EnterWriteLock ()) {
						while (_connectionMap.ContainsKey (id))
							id ++;
						cinfo = new ConnectionInfo (this, info.SubscribeInfo, id, sharedInfo, pubKey, payload, args.ReceiveEventHandler);
		...
		class SubscribeInfo : ISubscribeInfo
		{
			...
			public SubscribeInfo (AnonymousRouter2 router, Key key, ECKeyPair privateKey, int numOfRoutes, int numOfRelays, float factor)
			{
				_router = router;
				_key = key;
		...
		class TerminalPointInfo
		{
			...
			public TerminalPointInfo (AnonymousRouter2 router, Key recipientKey, RouteLabel label)
			{
				_router = router;
		...
		class ConnectionInfo
		{
			...
			public ConnectionInfo (AnonymousRouter2 router, SubscribeInfo subscribeInfo, Key destKey, ushort connectionId, DatagramReceiveEventHandler receivedHandler, AsyncCallback callback, object state)
			{
				_initiator = true;
				_router = router;
			...
			public ConnectionInfo (AnonymousRouter2 router, SubscribeInfo subscribeInfo, uint connectionId, byte[] sharedInfo,
				ECKeyPair publicKey, ConnectionEstablishPayload payload, DatagramReceiveEventHandler receivedHandler)
			{
				_initiator = false;
				_connected = true;
				_router = router;
			...
			public void Received (ConnectionReciverSideMessage msg)
			{
				...
				if (msg.Payload.Length > 0) {
					...
					_sock.InvokeReceivedEvent (new DatagramReceiveEventArgs (plain, plain.Length, null));
				}
			}
